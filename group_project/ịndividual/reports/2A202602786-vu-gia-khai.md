...
- **Nhóm:** Nhóm 2 thành viên (PTIT Admission RAG Pipeline - G02)
...
|---|---|---|:---:|
| **Task 1: Collect Legal Documents** | Thu thập và thẩm định 3 văn bản pháp lý chính sách tuyển sinh PTIT 2026 dạng PDF/DOCX; kiểm tra tính hợp lệ file size (>1KB). | `src/task1_collect_legal_docs.py`, `data/landing/legal/` | Done |
| **Task 2: Crawl News Articles** | Xây dựng crawler tự động với Playwright / BeautifulSoup cào 5 bài viết tin tức tuyển sinh, lưu trữ metadata chuẩn (`url`, `title`, `date_crawled`, `content_markdown`). | `src/task2_crawl_news.py`, `data/landing/news/` | Done |
